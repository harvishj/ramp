import { getEmployees, getTransactionsPaginated, getTransactionsByEmployee, setTransactionApproval } from './requests';
...
export function fakeFetch<TData, TParams extends object = object>(endpoint: RegisteredEndpoints, params?: TParams): Promise<TData> {
...
function mockApiLogger({ data, message = 'Success request', type = 'success' }: { message?: string; data: object; type?: 'success' | 'error' | 'info' }) {
...
function getTimeout() {
  const timeout = parseInt(new URL(document.location as unknown as URL).searchParams.get('timeout') ?? process.env.REACT_APP_TIMEOUT_MULTIPLIER ?? '1000');
...
export type RegisteredEndpoints = 'employees' | 'paginatedTransactions' | 'transactionsByEmployee' | 'setTransactionApproval';
